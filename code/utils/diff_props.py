...
class DiffGeomProps(nn.Module):
    ...
    def forward(self, xyz, uv):
        ...
        Args:
            xyz (torch.Tensor): 3D points, output 3D space (B, 3, Num_Points).
        ...
        for o in range(3):
            derivs = self.df(xyz[:, :, o], uv)  
            assert(derivs.shape == ( B, M, 2))
            dxyz_duv.append(derivs.contiguous().unsqueeze(2))
        # pdb.set_trace()
        # derivs=self.df(xyz,uv)
    ...
    def df(self, x, wrt):
        ...
        # wrt.requires_grad_(True)
        df_out=torch.ones(B*M, dtype=torch.float32, requires_grad=False).to(x.device)
