...
def evaluate(**kwargs):
    ...
    blur = kwargs['blur']
    fixed_transform= kwargs['fixed_transform']
    ...
    if eval_rendering:
        images_dir = '{0}/rendering-ep{1}-ts{2}'.format(evaldir, epoch,timestamp)
        unwarp_dir = '{0}/unwarp-ep{1}-ts{2}'.format(evaldir, epoch,timestamp)
        utils.mkdir_ifnotexists(images_dir)
        utils.mkdir_ifnotexists(unwarp_dir)
        ...
        for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
            ...
            for s in split:
                out = model(s)
                uv2_surf =model.backward_network(s['uv_inp'], None, global_feats=None)
                res.append({
                    'uv_points': out['uv_points'].detach(),
                    'rgb_values': out['rgb_values'].detach(),
                    'uv2surface_points':uv2_surf.detach() 
                })
            ...
            # pdb.set_trace()
            _3d_map_eval = model_outputs['uv2surface_points']
            _3d_map_eval = _3d_map_eval.reshape(batch_size, total_pixels, 3)
            pose=model_input['pose'].cuda()
            hom=torch.cat([_3d_map_eval, torch.ones(1,total_pixels,1).cuda()], dim=-1)
            hom_=hom.reshape(-1,4).transpose(1,0)
            pose_=torch.inverse(pose.reshape(4,4))
            # pose_=pose.reshape(4,4)
            intr=model_input['intrinsics'].cuda() 
            intr[0,1,1]=intr[0,0,0]
            camcorrd=torch.matmul(pose_, hom_)
            pixelcord=torch.matmul(intr,camcorrd[:3,:])
            pixelcord=pixelcord/pixelcord[0,2,:]
            # pdb.set_trace()
            alb=ground_truth['rgbhr']
            hrshape=alb.shape
            # pdb.set_trace()
            xx=pixelcord[0,0,:].reshape(img_res[0],img_res[1])/img_res[1]
            yy=pixelcord[0,1,:].reshape(img_res[0],img_res[1])/img_res[0]
            xx=cv2.resize(xx.cpu().numpy(),(hrshape[3],hrshape[2]))
            yy=cv2.resize(yy.cpu().numpy(),(hrshape[3],hrshape[2]))
            xx=torch.from_numpy(xx).cuda()
            yy=torch.from_numpy(yy).cuda()
            if blur:
                xx=cv2.blur(xx.cpu().numpy(),(17,17))
                yy=cv2.blur(yy.cpu().numpy(),(17,17))
                bm=np.stack([xx,yy], axis =-1)
                bm=torch.from_numpy(bm).cuda()
            else:
                bm=torch.stack([xx,yy],dim=-1)
            bm=(bm-0.5)*2
            alb=ground_truth['rgbhr']
            # alb=alb[0].transpose(1,0).reshape(3,img_res[0],img_res[1])
            # alb=alb[0].transpose(1,0).reshape(3,tex_res[0],tex_res[1])
            alb=alb/2.0+0.5
            uw=F.grid_sample(alb.cuda(), bm.unsqueeze(0), align_corners=True, padding_mode='border')
            # matplt.imshow(uw[0].transpose(0,1).transpose(1,2).detach().cpu())
            # matplt.show()
            uw=uw[0].transpose(0,1).transpose(1,2).detach().cpu().numpy()
            uw = Image.fromarray((uw * 255).astype(np.uint8))
            uw.save('{0}/uw_{1}.png'.format(unwarp_dir,'%03d' % indices[0]))
        ...
        log_string="RENDERING EVALUATION {2}: psnr mean = {0} ; psnr std = {1}".format("%.2f" % psnrs.mean(), "%.2f" % psnrs.std(), scan_id)
        print(log_string)
        with open('{0}/log-ep{1}-ts{2}.txt'.format(evaldir,epoch,timestamp),'w') as logfile:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--blur', default=False, action="store_true", help='If set, blur the bm prediction.')
    parser.add_argument('--prefwd_model', default=None, type=str, help='Specifies a fwd model to load from')
    parser.add_argument('--fixed_transform', default=False, action="store_true", help='Whether to apply fixed transformation to the 3d points')
    ...
    evaluate(conf=opt.conf,
             ...
             blur=opt.blur,
             fixed_transform=opt.fixed_transform
             )
